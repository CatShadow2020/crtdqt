...
StopAcquisitionThread::StopAcquisitionThread():QThread()
	{
	}
void StopAcquisitionThread::Init(MainFrame *p)
{
    ...
    QObject::connect(this, SIGNAL(disableToolbar(bool)),
                        p, SLOT(disableToolbarSlot(bool)));
...
void StopAcquisitionThread::run()
{
    ...
    emit disableToolbar(true);
	bContinue = FALSE;
	WaitForStop();
    emit disableToolbar(false);
    emit stopSignal();
}
